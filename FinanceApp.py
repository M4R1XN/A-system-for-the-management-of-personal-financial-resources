...
class FinanceApp(tk.Tk):
    ...
    def get_monthly_income(self):
        """Calculate total income for the current month in USD."""
        ...
        for _, row in income_transactions.iterrows():
            ...
            if currency == "USD":
                ...
                if converted_amount is not None:
                    total_income_usd += converted_amount
                else:
                    print(f"Warning: Could not convert {amount} {currency} to USD.")

        return round(total_income_usd, 2)

    def get_monthly_expenses(self):
        """Calculate total expenses for the current month in USD."""
        transactions = get_transactions(self.user_id, is_admin=self.is_admin)
        if not transactions:
            return 0.0
        df = pd.DataFrame(transactions)
    ...
    # Sum up expenses in USD
        ...
        for _, row in expense_transactions.iterrows():
            ...
            if currency == "USD":
                ...
                converted_amount = self.convert_currency(amount, currency, "USD")
            if converted_amount is not None:
                total_expenses_usd += converted_amount
            else:
                print(f"Warning: Could not convert {amount} {currency} to USD.")

        return round(total_expenses_usd, 2)

    def get_budget_usage(self):
        """Calculate the percentage of budget used for the current month."""
    ...
    def calculate_balance(self):
        """
        Calculate and display the balance, monthly income, expenses, and budget usage.
        """
    ...
    # Update the dashboard display
        ttk.Label(self.main_frame, text=f"Monthly Income: ${monthly_income:.2f}", font=("Helvetica", 12)).grid(row=1, column=0, sticky=tk.W)
        ttk.Label(self.main_frame, text=f"Monthly Expenses: ${monthly_expenses:.2f}", font=("Helvetica", 12)).grid(row=2, column=0, sticky=tk.W)
        ttk.Label(self.main_frame, text=f"Budget Used: {budget_usage:.2f}%", font=("Helvetica", 12)).grid(row=3, column=0, sticky=tk.W)

    # Notify if balance falls below the threshold
        total_balance_usd = sum(
            self.convert_currency(balance, currency, "USD") or 0
            for currency, balance in balance_by_currency.items()
        )
        if total_balance_usd < REMINDER_THRESHOLD:
    ...
    # Key Indicators Frame
        ...
        ttk.Label(
            frame_key_indicators, text=f"Monthly Income: ${self.get_monthly_income():.2f}",
            font=("Helvetica", 12)
        ).grid(row=1, column=0, sticky=tk.W, padx=5)

        ttk.Label(
            frame_key_indicators, text=f"Monthly Expenses: ${self.get_monthly_expenses():.2f}",
            font=("Helvetica", 12)
        ).grid(row=2, column=0, sticky=tk.W, padx=5)

        ttk.Label(
            frame_key_indicators, text=f"Budget Used: {self.get_budget_usage():.2f}%",
            font=("Helvetica", 12)
